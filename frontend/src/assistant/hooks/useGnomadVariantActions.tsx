...
import { useToolResult } from '@genohype/assistant-ui'
...
export function useGnomadVariantActions() {
  useCopilotAction({
    ...
    parameters: [
      { name: 'variant_id', type: 'string', description: 'The variant identifier (e.g., "1-55516888-G-A")', required: true },
      { name: 'dataset', type: 'string', description: 'The dataset to query (e.g., "gnomad_r4")', required: false },
    ],
  ...
  useCopilotAction({
    ...
    parameters: [
      { name: 'variant_id', type: 'string', description: 'The variant identifier', required: true },
      { name: 'dataset', type: 'string', description: 'The dataset to query', required: false },
    ],
    handler: async ({ variant_id, dataset }) => {
      return { variant_id, dataset }
    },
